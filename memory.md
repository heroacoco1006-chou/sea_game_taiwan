...
## [2026-07-20] 流行事件與大地圖情報約定

- 全世界同時維持一個「港口 × 非本港販售商品」流行，賣價 ×2，期限固定 90 天；一般每週行情事件不納入流行標示，避免大地圖過度雜亂。
- 大地圖必須同步提供三種辨識：港口呼吸光圈與「流行中」標籤、小地圖橘點、頂部跑馬燈（港口／商品／倍率／剩餘天數）。日期或事件輪替後需立即更新。
- 存檔 v21 將舊存檔當時的 30 天流行一次延長 60 天，後續不得在每次 refresh 時反覆展延。

## [2026-07-19] 公開網站版本與 Git 標籤約定

- 六角格海戰正式啟用是 V1／V2 分水嶺：`v1.0.0` 固定指向 `2474f97`（經典海戰最後正式流程），V2 從 `676b35d` 正式切換六角海戰，首個穩定標籤為 `v2.0.0`。
- 玩家可見版號唯一程式來源為 `src/version.ts`；入口底部必須顯示版本號與版本名稱。發版時同步 `package.json`、`package-lock.json`、`VERSION.md` 與 Git annotated tag。
- 版本號採語意化概念：錯誤／平衡／小內容修正升第三碼，新完整系統升第二碼，再次更換核心玩法世代才升第一碼。公開網站每次形成可辨識版本都要留 tag 與版本說明。
...
- 自動戰鬥只是由 P6 AI 代替玩家送 `BattleCommand`，雙方必須共用同一 engine 與 seed；啟用後不因中途受傷強制關閉。事件加速播放，但每個我方新回合須保留 1.2 秒接手窗口；接手後立即停止排程並恢復完整手動操作。
- P6-2 固定回歸必含 1v1、2v3、5v5 與門檻兩側；正式入口維持 `USE_HEX_BATTLE=false`，直到 P7 四項艦隊／遭遇閘門完成。

## [2026-07-13] P7 艦隊數量與敵軍分級定案（老闆核准，P7 開工前必查）

- 一般海戰中，玩家 `state.ship`＋`state.escorts` 必須一艘對應一個戰鬥單位，實際擁有 1～5 艘就進場 1～5 艘；P3～P6 固定 5 對 5 只屬預覽資料，不得沿用到 P7 adapter。
- 一般海盜船數機率資料化：tier 1＝1 艘 80%／2 艘 20%；tier 2＝2 艘 60%／3 艘 40%；tier 3＝3 艘 40%／4 艘 60%。具名主線／夥伴戰不隨機，資料可指定 1～5 艘，大型戰役採 4～5 艘。
- 各 tier 共用合法 AI；難度由船數、船型、耐久、砲數、水手、裝備與地圖提升，不用故意犯錯模擬低級 AI。
- 劇情明示有友軍艦隊而玩家船數不足時，可補臨時故事友軍至我方最多 5 艘；友軍不寫入存檔、不成為戰利品、不取代玩家船。P7 開工前必須先建立遭遇資料、adapter 與固定 seed 驗收，四項未接齊不得切正式入口。
...
- 世界地圖方向與情境動作必須由鍵盤與觸控共用同一套 movement／action handler；港町保留點擊移動並提供方向／進入備援，UI 點擊不得穿透成移動。
- 網頁容器必須宣告 `touch-action: none` 與 overscroll 防護；桌面方向鍵／WASD／Enter／滑鼠操作必須保留。
