...
1. **回合方式**：採「玩家全隊後敵方全隊」（本案建議），或每艘船依速度交錯行動。
2. **方向與側舷**：第一版就做 6 朝向＋側舷射界（本案建議），或先簡化成全方向砲擊。
3. **戰場大小**：11×7（本案建議），或希望更大、更接近附件的大艦隊感。
4. **接舷結果**：俘獲普通敵船只換戰利品（本案建議），或允許戰後把敵船加入艦隊。
5. **回合上限**：12 回合（本案建議）是否合適。
6. **第一版地形**：深海／淺灘／島嶼／暗礁是否足夠。
7. **自動戰鬥**：第一版先不做（本案建議），或必須與新系統一起上線。
