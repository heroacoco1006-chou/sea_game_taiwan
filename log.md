...
## [2026-07-20] 功能 | 操作者：Codex | 三個月流行與大地圖流行情报

- 依老闆定案將單一全球流行由 30 天延長為 90 天；存檔升 v21，舊存檔現有流行一次延長 60 天。
- 流行港口新增大地圖橘色呼吸光圈與「🔥 流行中」標籤，小地圖同步顯示閃爍橘點。
- 頂部個人狀態列下方新增跑馬燈，顯示港口、商品、賣價 ×2 與剩餘天數，日期推進與流行輪替時自動更新。
- 新增流行 90 天、到期日仍有效及隔日輪替測試；production build 通過。
- 依公開發版約定升為 V2.1.0。

## [2026-07-20] 修正 | 操作者：Codex | 更正日本港町彩色箭頭判讀

- 重新比對 `日本港口地圖設施位置修正.png` 與 `日本港口地圖設施位置修正_2.png`，確認箭頭尖端指向有設施建築的彩色圈；V2.0.1 誤把空圈當成終點。
- 酒館、旅館、商館、交易所與道具屋恢復至原始標記圖箭頭所指位置；造船廠與港口維持原位，並撤回不必要的日本可走區擴張。
- 新增 5 項日本港町標記座標回歸測試，並以 layout validator 與 production build 驗證。
- 依公開發版約定將更正版升為 V2.0.2。

## [2026-07-19] 修正 | 操作者：Codex | 日本南洋港町定位與大地圖左右航向

- 依 `need_to_fix` 彩色標記圖調整日本 5 處、南洋 4 處設施座標；日本左上目標點同步納入可走區，避免設施可見但無法抵達。
- 大地圖方向圖的左右原始圖片與標示相反，改由獨立方向判定函式交換純水平 frame；上下與垂直為主的斜向維持原判定。
- 新增 6 組方向回歸測試；港町 layout validator 與 production build 一併驗證。
- 依公開發版約定將修正版升為 V2.0.1。

## [2026-07-19] 文件 | 操作者：Codex | 建立 V1／V2 版本制度與入口版號

- 以六角海戰正式啟用為分水嶺：`v1.0.0` 指向經典海戰最後版本 `2474f97`；目前版本定為 `v2.0.0`「六角海戰版」。
- 新增 `VERSION.md`，整理兩代主要內容與後續發版規則；README 加入版本沿革入口，npm package 版號同步升為 2.0.0。
- 新增 `src/version.ts` 作玩家可見版號來源，遊戲標題入口底部顯示 `V2.0.0｜六角海戰版`。
- 長期約定寫入 memory：公開發版同步程式版號、package、VERSION 文件與 Git annotated tag。

## [2026-07-19] 修正 | 操作者：Codex | 台灣港町依彩色標記調整設施定位

- 依 `need_to_fix/台灣港口地圖_設施位置修正.png` 判讀：酒館移至藍圈、交易所移至綠圈、港口入口移至紅圈；各設施互動門點同步更新。
- 港口角色出生點隨紅圈位置調整，避免入港後仍出現在舊碼頭；大員唯一使用的造船廠為避開新酒館位置，移至上方中央空地。
- 驗證：台灣版型七設施無矩形重疊、港町 layout validator 通過、production build 通過。

## [2026-07-19] 修正 | 操作者：Codex | need_to_fix 港町與造船廠四項畫面修正

- 依 `need_to_fix/` 四張截圖判讀並修正：日本港酒館／交易所移至空地，造船廠同步右移避免設施互疊；南洋港口入口、互動門點與角色出生點一起左移。
- 精緻港町背景已內建碼頭船隻，因此不再重複疊加世界地圖用船圖；程序繪製 fallback 港町仍保留玩家船。
- 造船廠船型詳情上移、縮小字級與行距並啟用進階換行，船卡同步上移，避免說明文字蓋到底部建造按鈕。
- 驗證：港町 layout validator 通過、production build 通過、`git diff --check` 通過；本次內建瀏覽器沒有可用分頁，待老闆更新版本後人工覆核截圖位置。
