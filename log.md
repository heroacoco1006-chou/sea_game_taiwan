...
## [2026-07-19] 修正 | 操作者：Codex | 試玩回饋：任務接取港、海戰強度與入港休整

- 一般委託資訊新增「接取港口」，海戰與探索委託並直接寫明應回哪一港的官府／商館領賞。
- 一般海盜強度不再只按遊戲天數升級，改由航海天數、船長等級與艦隊船數共同限制；Lv1 單船即使累積很多天仍為 tier 1（開闊海域、敵軍 1～2 艘），成長後才逐步進入島嶼水道與礁岩航道。
- 六角格海戰標題移除玩家可見的「P8 美術整合」開發代號。
- 入港時會清除鼠患、暴風雨、思鄉病等海上持續負面狀態；消耗品仍可供航行途中緊急解除。
- 驗證：P7／試玩回饋測試 7 組全過，`npm run build` production build 完成；未更動既有使用者文件與素材變更。

## [2026-07-18] 開發 | 操作者：Codex | 聲望特殊事件與三件稀有道具完成

- 老闆確認規劃 1～5 後實作三軌聲望各 40／80，共 6 個一次性特殊事件；事件獨立於主線、普通委託與夥伴任務，沒有期限或永久失敗。
- 新增交付、探索、具名決鬥三類流程，官府／商館提示、Story 開場／結局、任務日誌下一步、六角格與 legacy 海戰結算均已接通。
- 新增海商信用牌（交易 7%）、航海星盤（探索 +8%）、守航水師佩刀（接舷 +18）；不進商店、只發一次、裝備頁以 ★ 標示。六分儀因年代不符改用航海星盤。
- 存檔升 v20；圖鑑由生成器增至 126 筆。新增資料 validator 與事件引擎測試，並更新既有 v19 版本斷言。
- 驗證：全 validators、全 `test*.mjs`、`npm run build`、本機首頁／新遊戲／世界圖／任務頁／裝備頁與 console 0 error 全過。
- 老闆同步回報既有驗收清單 11～17、19、20 正常；1～10、18 尚未測試。


## [2026-07-17] 修正 | 操作者：小航 | 檢討報告修正包：A類錯誤＋平衡調整＋反悔移動機制

- 依老闆對檢討報告的逐項決策實作（B-3 不修）：
  1. **A-1 水手蒸發**：賣僚艦超編水手上岸＋提示；海戰結算改「進場−存活」差值法扣水手；結算顯示折損人數（C-2）。
  2. **A-2 死任務日誌**：資訊頁過濾已錯過的夥伴任務，改列「⏳ 已錯過」一行。
  3. **A-3＋B-1③ 俘船換戰利品**：投降敵船依耐久上限 ×1.5 折算額外戰利品（資料化 capture.perHullMax），結算顯示俘獲艘數與金額。
  4. **B-1① 接舷門檻**：目標耐久需 ≤70%（boarding.requireHullBelow）才能跳幫，新 error TARGET_TOO_STURDY；AI 經同一 validate 自動遵守（新增滿血不接舷 AI 測試）。
  5. **B-2 撤退整補費**：主動撤退（PLAYER_RETREATED）扣資金 5%、封頂 1500 兩；回合耗盡脫離不收費。
  6. **B-4 經驗曲線**：100+(L−1)×60 → 80+(L−1)×20，通關落點 Lv25~30；舊檔免遷移。
  7. **B-5 聲望被動**：商人聲望→賣價最多 +5%；冒險名聲→探索發現率最多 +10%；海上威名→海盜遭遇最多 −50%（任務決鬥不受影響）；人物頁顯示被動數值。特殊事件＋稀有道具構想記入 status 待辦。
  8. **B-6 修整改版**：單次 10%（3~25），每回合可修（repaired 每回合重置），累計配額＝耐久上限 25%（repairUsed），用罄按鈕灰階＋REPAIR_EXHAUSTED。
  9. **新機制：反悔移動**（老闆提案）：移動後、行動前顯示「↩ 反悔移動」按鈕，退回原位原朝向、移動力退還、可重新規劃；原位被佔回 UNDO_BLOCKED；每回合重置快照。
- 存檔結構未動（v19）；battleRules.json 新增 4 個資料段，全部數值可調。
- 驗證：引擎測試 22 組（含新增修整配額／反悔／接舷門檻案例）、AI 10 組（含新增滿血不接舷）、自動戰鬥 4 組、P7 5 組、120 場模擬（無死循環、12 回合內結算）、全 validators、production build 全過；preview 實測反悔按鈕全流程、修整配額灰階、接舷門檻文案。settle 俘船／撤退費 node 驗證腳本待 shell 恢復後補跑。
- 待老闆試玩驗收（status 待辦 P1）。

## [2026-07-16] 閱讀分析 | 操作者：小航 | 全面檢討報告（程式邏輯＋平衡性，未動程式）

- 依老闆指示最高層級檢視全專案：state.ts、六角海戰六檔全讀、五個關鍵場景抽讀、資料檔數值核算與 XP／利潤試算。產出 `2026-07-16_遊戲全面檢討報告.md`。
- 🔴 確定錯誤 3 件：A-1 賣僚艦不裁水手→海戰後水手無聲蒸發（ShipyardScene.sellEscort＋battleAdapter）；A-2 資訊頁任務日誌不濾過期任務（限時錯過者永遠掛著）；A-3 老闆核准的「俘船換戰利品」未實作（投降與擊沉獎勵無差異）。
- 🟡 平衡問題 6 件：接舷壓倒性最優（decisive 1.25 vs 玩家水手優勢）、撤退零成本 vs 戰敗 −30% 重罰的怪異組合、12 回合躲贏拿全額戰利品、Lv50 實際不可達（通關約 Lv15-17）、聲望達標後無後續用途、海戰修整指令無用（5%/12 點 vs 每輪 40-70 傷害）。
- 🔵 品質觀察 8 件（rangeModifiers 型別掩蓋、被擊沉船水手損失無回饋、seed 可預測等）。
- 結論：**可以收尾**——無崩潰級問題；建議收尾前修 A-1／A-2／A-3（各 1-2 小時），戰鬥手感類（B-1/2/6）留待上線後獨立平衡包，多數可用 battleRules.json 資料值調整。
- 未修改任何遊戲程式。
...
- P9 裝置矩陣與節奏兩項人工閘門改列通過；總驗收由 20／22 更新為 22／22，0 待驗、0 失敗。
- P9 驗收完成不等同自動切正式入口；`USE_HEX_BATTLE=false` 與舊 `BattleScene` 回退仍保留，待老闆明確核准後才另段更新建構書與正式旗標。

---

## [2026-07-15] 修正 | 操作者：Codex | P9 觸控目標與窄螢幕排列

- 新增 `touchTargets.ts`，依 Phaser Scale.FIT 後的 canvas CSS 寬高反算最小邏輯 hit area；共用 `setMinimumCssTouchTarget()` 只擴大可點範圍，不改桌面按鈕外觀。
- `BattleHexScene` 登錄地圖與 13 個指令控制，resize 時重新計算；縮放低於 0.75 時改用手機橫向緊湊排列，正式遭遇隱藏無效地圖切換列，並提供唯讀 audit 資料。
- 新增 P9 靜態與數值測試：桌面 44.00、iPad 44.25、手機 44.42、小手機 44.38 CSS px。P1～P8 回歸、120 場固定模擬、lazy load、UI、主線、夥伴、觸控與 production build 全數通過。
- 老闆完成目前畫面測試並回覆正常；P9 更新為 20 通過／2 待人工／0 未通過。手機／iPad 三類遭遇完整單指戰鬥與一般戰鬥 3～5 分鐘計時仍保留待驗，`USE_HEX_BATTLE=false` 不變。

---

## [2026-07-15] 閱讀分析 | 操作者：Codex | P9 第一輪總驗收

- 依施工規格 §八／§九建立 `2026-07-15_P9_六角格海戰總驗收清單.md`；22 項結果為 19 通過、2 待人工矩陣、1 未通過，故未切正式入口、未改《遊戲建構書》§5-5。
- 必跑 validators、P1 六角數學 12 組、P2～P5 引擎 20 組、P6 AI、P6-2 自動戰鬥、P7 adapter／存檔、P8 素材、120 場模擬與 `npm run build` 全部通過。Node 24 的 TS／JSON 專測須帶既有 loader。
- 本機四入口皆 HTTP 200；Pages run `29331321047`（`7a8b843`）success，首頁、主程式與代表海戰素材均 HTTP 200。
- BattleHex 38／42／44 邏輯 px 按鈕經 FIT 縮放後無法保證 44 CSS px，列為未通過；手機／iPad 單指完整三場與 3～5 分鐘列待人工。畫面控制工具授權逾時，未把未完成矩陣誤記通過。`USE_HEX_BATTLE=false` 不變。


## [2026-07-14] 修正 | 操作者：Codex | P8 耐久條與船首朝向辨識改善

- 依老闆實測回饋，耐久條改為 8px 高對比條：健康螢光亮綠、受損亮黃、危急亮紅；素材外框先畫、色條後畫，避免外框半透明底讓顏色變灰，已行動船仍保留 86% 可見度。
- 每艘在場船新增黑邊金色船首箭頭，直接依 engine 的六方向 `facing` 計算位置；實際按左／右轉後箭頭同步換向。資訊面板文案由「朝向」改為「船首朝向」。
- P8 validator 新增亮綠色、船首箭頭與船首文案固定檢查。P1 12 組、P2～P5 20 組、P8 素材、lazy load、觸控、15 場景 UI 與 TypeScript 全數通過；production build 成功（461 modules，僅既有大型 chunk 提醒）。
- 桌面 1280×720 實際選船並右轉、iPad 1180×820 劇情案例均確認耐久條與箭頭清晰，無頁面溢位，當次 console 0 error；`USE_HEX_BATTLE=false` 不變。
